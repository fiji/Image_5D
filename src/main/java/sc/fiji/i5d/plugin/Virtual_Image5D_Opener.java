...
public class Virtual_Image5D_Opener implements PlugIn {
	...
	public void run(final String arg) {
		...
		final OpenDialog od =
			new OpenDialog("Open Sequence of Images Stacks...", arg);
...
//      selectedFilename = name;        
		...
		try {
			...
			for (int i = start - 1; i < list.length; i++) {
				...
				if (stack == null) imp = new Opener().openImage(directory, list[i]);
		...
		final I5DVirtualStack newStack =
			new I5DVirtualStack(stack.getWidth(), stack.getHeight(), stack
				.getColorModel(), directory);

		// Sort: loop with channel changing fastest. Read appropriate slice filename
		// from stack and
		...
		final Image5D img5d =
			new Image5D("virtual", newStack, nChannels, nSlices, nFrames);
	...
	boolean showDialog(final ImagePlus imp, final String[] list) {
		...
		final Vi5dOpenerDialog gd =
			new Vi5dOpenerDialog("Sequence Options", imp, list);
	...
	class Vi5dOpenerDialog extends GenericDialog {
		...
		public Vi5dOpenerDialog(final String title, final ImagePlus imp,
			final String[] list)
		{
			super(title);
			this.imp = imp;
...
//            int nSlices = imp.getStackSize();
			...
			if (n2 < 0) n2 = 0;
			final double size =
				(double) (width * height * n2 * bytesPerPixel) / (1024 * 1024);
			((Label) theLabel).setText(width + " x " + height + " x " + n2 + " (" +
				IJ.d2s(size, 1) + "MB)");
