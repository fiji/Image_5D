...
public class Transfer_Channel_Settings implements PlugIn {
	...
	public void run(final String arg) {
		...
		final Image5D[] i5dList = new Image5D[idList.length];
		int nI5Ds = 0;
		...
		for (int n = 0; n < idList.length; n++) {
				...
				idList[n] != imp.getID())
			{
				...
				if (idList[n] == choiceID) choiceTitle =
					WindowManager.getImage(idList[n]).getTitle();
				nI5Ds++;
			}
		}
		...
		final String[] i5dTitles = new String[nI5Ds];
		for (int n = 0; n < nI5Ds; n++) {
			i5dTitles[n] = i5dList[n].getTitle();
		}

		final GenericDialog gd = new GenericDialog("Transfer Channel Settings");
		...
		final Image5D src = i5dList[gd.getNextChoiceIndex()];
		final Image5D dest = (Image5D) imp;

		choiceID = src.getID();

		final int nChannels = Math.min(src.getNChannels(), dest.getNChannels());

		src.storeCurrentChannelProperties();
		dest.storeCurrentChannelProperties();
		for (int c = 1; c <= nChannels; c++) {
			if (transferColors) {
				dest.setChannelColorModel(c, src.getChannelDisplayProperties(c)
					.getColorModel());
			}
			if (transferLabels) {
				dest.getChannelCalibration(c).setLabel(
					src.getChannelCalibration(c).getLabel());
			}
			if (transferCalibrations) {
				// Make deep copy of ChannelCalibration to avoid messing up the function
				// of the src.
				final ChannelCalibration chCal = src.getChannelCalibration(c).copy();
				dest.getChannelCalibration(c).setFunction(chCal.getFunction(),
					chCal.getCoefficients(), chCal.getValueUnit(), chCal.isZeroClip());
			}
		}
